...
export default {
    ...
    created() {
        const recipient = this.$store.getters['delivery/deliveryInfo']?.recipient
        if (recipient) {
            this.form = { ...recipient }
        }
    },
    watch: {
        form: {
            handler(newVal) {
                this.$store.dispatch('delivery/updateRecipient', newVal)
            },
            deep: true,
        },
    },
    mounted() {
        this.$emit('update-title', 'Отправитель')
    },
}
</script>